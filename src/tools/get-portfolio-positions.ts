...
const getPortfolioPositionsParams = z.object({
	apiKey: z
		...
		.optional()
		.describe(
			"API key or bearer token for authentication (required for portfolio access)",
		),
});
...
export const getPortfolioPositionsTool = {
	...
	description:
		"Get your active portfolio positions on Limitless with P&L calculations and market values. Requires authentication via API key or session token.",
	...
	execute: async (params: GetPortfolioPositionsParams) => {
		try {
			...
			const portfolio = await service.execute(params.apiKey);
